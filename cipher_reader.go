...
// NewCipherReader creates CipherReader wrapped around provided Reader. The associated cipher is created from the provided CipherConstructor, key and iv. The optional buffer is used internally. If buffer is not provided, it will be created with DefaultBufferSize.
...
// Read reads necessary amount of input from the underlying Reader and decrypts it into the provided slice. It conforms to the Reader interface.
// Note that due to the nature of block ciphers, certain amount of read-ahead is necessary to provide the requested amount of bytes, although best effort is made to minimize the amount of read-ahead (generally only the input necessary to decrypt the last partially read block).
